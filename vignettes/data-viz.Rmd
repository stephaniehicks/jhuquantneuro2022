...
```{r}
chicago <- dplyr::as_tibble(chicago)
str(chicago)
```
...
```{r, eval=FALSE}
dplyr::select(chicago, -(city:dptp))
```
...
```{r}
subset <- dplyr::select(chicago, ends_with("2"))
str(subset)
```
...
```{r}
subset <- dplyr::select(chicago, starts_with("d"))
str(subset)
```
...
```{r}
chic.f <- dplyr::filter(chicago, pm25tmean2 > 30)
str(chic.f)
```
...
dplyr::select(chic.f, date, tmpd, pm25tmean2)
```
...
```{r}
chicago <- dplyr::arrange(chicago, date)
```

We can now check the first few rows

```{r}
head(dplyr::select(chicago, date, pm25tmean2), 3)
```

and the last few rows.

```{r}
tail(dplyr::select(chicago, date, pm25tmean2), 3)
```

Columns can be arranged in descending order too by useing the special `desc()` operator.

```{r}
chicago <- dplyr::arrange(chicago, desc(date))
```
...
```{r}
chicago <- dplyr::mutate(chicago, year = as.POSIXlt(date)$year + 1900)
```

Now we can create a separate data frame that splits the original data frame by year.

```{r}
years <- dplyr::group_by(chicago, year)
```

Finally, we compute summary statistics for each year in the data frame with the `summarize()` function.

```{r}
dplyr::summarize(years, pm25 = mean(pm25, na.rm = TRUE), 
...
```{r}
dplyr::mutate(chicago, year = as.POSIXlt(date)$year + 1900) %>%    
        dplyr::group_by(year) %>% 
        dplyr::summarize(years, pm25 = mean(pm25, na.rm = TRUE), 
          o3 = max(o3tmean2, na.rm = TRUE), 
          no2 = median(no2tmean2, na.rm = TRUE))
...
```{r}
dplyr::slice_sample(chicago, n = 10)
```
...
```{r}
dplyr::slice_head(chicago, n = 5)
```

This will show the last 5 rows.

```{r}
dplyr::slice_tail(chicago, n = 5)
```
